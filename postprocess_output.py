...
if __name__ == '__main__':
    ...
    def uncertainty_file(site: str) -> Iterator[tuple[str, Path]]:
        """ Return an iterator over uncertainty including variable names and file paths"""
        directory = os.path.join(os.environ['SHARDIR'], '../zdr', 'CalLMIP')
        for entry in os.scandir(directory):
            if not entry.is_file():
                continue
            path = Path(entry.path)
            if site not in path.stem or path.suffix.lower() != ".csv":
                continue
            stem = path.stem # file name without extension
            sep = stem.rfind("_ICB20TR_")
            if sep == -1:
                continue
            varname = stem[sep + 9:]
            yield varname, path
    ...
    if callable(uncertainty_file) or uncertainty_file.endswith('.nc'):

        # Read full uncertainty arrays (covers a superset of the output time ranges).
        if callable(uncertainty_file):
            uc_data = {}
            for in_name, file in uncertainty_file(site_name):
                temp = np.loadtxt(file, delimiter=",")
                # 23936 MCMC samples x 657 10-day periods
                temp = np.nanstd(temp, axis = 0)
                # convert 10-day average uncertainty to daily uncertainty
                uc_data[in_name] = np.repeat(temp, 10) * np.sqrt(10)
                # gC/m2/day -> gC/m2/s
                if (in_name == 'NEP'):
                    uc_data[in_name] /= 86400

            start = cftime.DatetimeNoLeap(1997, 1, 1)
            uc_dates = np.array([start + timedelta(days=i) for i in range(365*(2014-1997+1))])

        elif uncertainty_file.endswith('.nc'):
            uc_data = {}
            with Dataset(uncertainty_file, "r") as src:
                for in_name in src.variables:
                    uc_data[in_name] = src[in_name.remove("_uc")].values
        ...
        # Convert the unit convention to be consistent with output requirements
        uc_specs = [
            ("NEP", ALMA_VARS[0]),
            ("EFLX_LH_TOT", ALMA_VARS[1]),
            ("FSH",  ALMA_VARS[2]),
        ]
        uc_arrays_full = {in_name: np.abs(spec.compute(uc_data[in_name])) for in_name, spec in uc_specs}
        ...
        for out_path in [
            ...
            for in_name, spec in uc_specs:
                arr = np.asarray(uc_arrays_full[in_name][uc_idx], dtype=np.float32).reshape(-1, 1, 1)
                v = dst.createVariable(
                    f"{spec.alma_name}_uc", "f4", ("time", "lat", "lon"),
                    fill_value=np.float32(1.0e36),
                    zlib=True, complevel=4,
                )
                v.units = spec.units
